<? if (!empty($image_and_text_widget)): ?>
    <div class="widget content content__image_text">
        <div class="container">
            <div class="row">
                <? foreach ($image_and_text_widget as $widget):
                    ...
                    <div class="col-sm-12 <?= isset($columns) ? $columns : '' ?>">
                        <div class="widget__container">
                            <div class="content__image_text__image mb-3">
                                <? if (!empty($widget['image'])): ?>
                                    <img alt="<?= $widget['title'] ?>"
                                         src="<?= fileSrc($widget['image']) ?>">
                                <? endif ?>
                            </div>
                            <div class="content__image_text__container">
                                <? if (!empty($widget['headline'])): ?>
                                    <h5 class="widget__headline content__image_text__headline">
                                        <?= $widget['headline'] ?>
                                    </h5>
                                <? endif ?>
                                <? if (!empty($widget['title'])): ?>
                                    <h2 class="widget__title content__image_text__title">
                                        <?= $widget['title'] ?>
                                    </h2>
                                <? endif ?>
                                <? if (!empty($widget['text'])): ?>
                                    <div class="content__image_text__text">
                                        <?= $widget['text'] ?>
                                    </div>
                                <? endif ?>
                                <? if (isset($widget['button']) && !empty($widget['button']['title'])): ?>
                                    <div class="content__image_text__button_group">
