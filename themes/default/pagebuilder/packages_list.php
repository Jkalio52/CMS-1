<? if (!empty($packages_list)): ?>
    <div class="widget content packages_list">
        <div class="container">
            <div class="row">
                <? foreach ($packages_list as $widget):
                    ...
                    <div class="col-sm-12 <?= $columns ?>">
                        <div class="widget__container packages_list__container text-<?= isset($widget['text_align']) ? $widget['text_align'] : '' ?>">
                            <? if (!empty($widget['title'])): ?>
                                <h2 class="widget__title packages_list__title mb-3">
                                    <?= $widget['title'] ?>
                                </h2>
                            <? endif ?>
                            <? if (!empty($widget['price'])): ?>
                                <h5 class="widget__price packages_list__headline mb-2">
                                    <?= $widget['price'] ?>
                                </h5>
                            <? endif ?>
                            <? if (!empty($widget['description'])): ?>
                                <div class="widget__text packages_list__text">
                                    <p><?= $widget['description'] ?></p>
                                </div>
                            <? endif ?>
                            <? if (isset($widget['button']) && !empty($widget['button']['title'])): ?>
                                <div class="widget__button_group packages_list__button_group">
