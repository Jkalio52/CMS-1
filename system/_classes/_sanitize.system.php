...
namespace _WKNT;
class _SANITIZE extends _INIT
{
    /**
     * Sanitize input type
     */
    static public function input($obj)
    {
        $new_obj = [];
        if (is_array($obj)):
            if (!empty($obj)):
     ...
     */
    static public function textarea($obj)
    {
        $new_obj = [];
        if (is_array($obj)) {
            foreach ($obj as $key => $VALUE) {
                $new_obj[$key] = htmlentities($VALUE, ENT_QUOTES, "UTF-8");
            }
        } else {
            $new_obj = htmlentities($obj, ENT_QUOTES, "UTF-8");
        }
     ...
     * Textarea decode
     */
    static public function textarea_decode($obj)
    {
        return html_entity_decode($obj, ENT_QUOTES, "UTF-8");
    }
     ...
     * @return  str
     */
    static public function encode_php_tags($obj)
    {
        return str_replace(array('<?php', '<?PHP', '<?', '?>'), array(
            '&lt;?php',
            '&lt;?PHP',
            '&lt;?',
            '?&gt;'
     ...
     * @return string
     */
    public static function pageBuilder($string)
    {
        return strtolower(trim(preg_replace('~[^0-9a-z]+~i', '_', html_entity_decode(preg_replace('~&([a-z]{1,2})(?:acute|cedil|circ|grave|lig|orn|ring|slash|th|tilde|uml);~i', '$1', htmlentities($string, ENT_QUOTES, 'UTF-8')), ENT_QUOTES, 'UTF-8')), '-'));
    }
     ...
     *
     */
    public function remove_accents($obj)
    {
        if (!preg_match('/[\x80-\xff]/', $obj)) {
            return $obj;
        }

        if (self::seems_utf8($obj)) {
            $chars = array(
            ...
            $obj                 = strtr($obj, $chars['in'], $chars['out']);
            ...
            $obj                 = str_replace($double_chars['in'], $double_chars['out'], $obj);
        }
     ...
     *
     */
    public function seems_utf8($obj)
    {
        $length = strlen($obj);
        for ($i = 0; $i < $length; $i++) {
            $c = ord($obj[$i]);
            ...
            for ($j = 0; $j < $n; $j++) { # n bytes matching 10bbbbbb follow ?
                if ((++$i == $length) || ((ord($obj[$i]) & 0xC0) != 0x80)) {
                    return false;
                }
            }
        }
